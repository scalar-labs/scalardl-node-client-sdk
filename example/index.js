...
(async () => {
  ...
  try {
    let r;
    r = await clientService.registerCertificate();
    console.log(`${r.getStatus()}: ${r.getMessage()}`);

    const fs = require('fs');
    const buffer = fs.readFileSync(__dirname + '/ListTypeContract.class');

    r = await clientService.registerContract('ListTypeContract',
        'com.scalar.am.contract.ListTypeContract',
        buffer, {holderId: 'foo@example.com'});
    console.log(`${r.getStatus()}: ${r.getMessage()}`);

    r = await clientService.listContracts();
    console.log(`${r.getStatus()}: ${r.getMessage()}`);

    r = await clientService.executeContract('ListTypeContract', {});
    console.log(`${r.getStatus()}: ${r.getResult()}`);
  } catch (e) {
    console.log(e);
  }
})();
